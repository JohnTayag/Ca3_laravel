...
<div class="w-4/5 m-auto pt-20">
    ...
    <h2 class="mt-6 text-4xl leading-10 tracking-tight font-bold text-blue-900 text-center">Comment Section</h2>
    <div>
        <form action="/blog/{{ $post->id }}/comments" method="POST" class="mb-0">
            ...
            <textarea name="text" class="mt-3 py-4 px-3 block w-full borded border-blue-400 rounded-md shadow-sm" required>{{ old('text') }}</textarea>
            ...
            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Post</button>
        </form>
                ...
                <div class="mb-5 bg-white px-4 py-6 rounded-sm shadow-md">
                    <div class="flex">
                        ...
                        <div class="flex flex-col justify-center">
                            <p class="mr-2 text-blue-600 font-bold">{{ $comment->author }}</p>
                            <p class="text-blue-600">{{ $comment->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    ...
                    <form action="/comments/{{ $comment->id }}" method="POST" class="mb-0 mt-3">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class=" bg-blue-500 text-gray-100  py-1 px-1 rounded-3xl">Delete comment</button>
                    </form>
                </div>
            @endforeach
